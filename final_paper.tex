...
\begin{abstract}
As one of the significant industries in American entertainment, the NBA franchises have significantly increased, averaging \$4.66 billion each in 2025, with predictions for continued growth.  This rising valuation underscores the escalating financial stakes and intensifying labor market competition in professional basketball. This project aimed to determine which performance metrics most accurately predict NBA player salaries. We constructed and compared the Ordinary Linear Regression (OLS) model, the Random Forest model, and the Ridge and Lasso regression models to find the model that could best predict the log player salary. Across all models, we determined that the Lasso model has the best prediction accuracy, with its lowest RMSE (0.599) and $R^2$ (0.596). We found that Usage Percentage (\texttt{USG\%}), Age (\texttt{Age}), Minutes Played (\texttt{MP}), and Box Plus/Minus (\texttt{BPM}) are the strongest four positive predictors, which also consistently explain the salary variation among all the models we tested. 
