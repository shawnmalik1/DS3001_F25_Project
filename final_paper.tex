...
We partitioned the data into an 80\% training set and a 20\% held-out test set. Within the training data, we performed 5-fold cross-validation to tune hyperparameters. For Ridge and Lasso regression, we selected the regularization strength ($\alpha$) that minimized cross-validated error. For the Random Forest, we tuned a small grid of depth and tree count settings to avoid overfitting. This validation setup balanced bias and variance and gave a reasonable estimate of out-of-sample performance.
